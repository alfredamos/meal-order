...
export default function StripeCheckout({amount}: Props) {
  ...
  /* const {mutate, data} = useStripePayment()
  mutate(convertToSubCurrency(amount)); */
  useEffect(() => {
    stripeService.payment(convertToSubCurrency(amount)).then(data => setClientSecret(data.clientSecret as unknown as string))
  },[amount])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const {error} = await stripe.confirmPayment({
      elements, clientSecret, confirmParams: {return_url: `http://www.localhost:3000/payment-success?amount=${amount}`}
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-md">
      ...
      <button disabled={!stripe || loading} className="text-white w-full p-5 bg-black mt-2 rounded-md font-bold disabled:opacity-50 disabled:animate-pulse">
        {!loading ? `Ppay ${amount}` : "Processing..."}
      </button>
    </form>
  )
}